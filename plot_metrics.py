...
def plot_metrics(csv_path: str = 'matmed_metrics.csv', output_path: str = 'matmed_training_plot.png'):
    ...
    # We expect columns: episode, avg_reward, best_reward, pct_valid, diversity, total_loss
    ...
    # 1. Rewards
    axs[0, 0].plot(df['episode'], df['avg_reward'], label='Avg Reward', color='blue', alpha=0.7)
    axs[0, 0].plot(df['episode'], df['best_reward'], label='Best Reward', color='green', linewidth=2)
    ...
    # 2. Validity
    axs[0, 1].plot(df['episode'], df['pct_valid'], color='purple', linewidth=2)
    ...
    # 3. Diversity
    axs[1, 0].plot(df['episode'], df['diversity'], color='orange', linewidth=2)
    ...
    if 'total_loss' in df.columns:
        axs[1, 1].plot(df['episode'], df['total_loss'], color='red')
